/**
 * AUD-066 音頻帶通濾波 - Audio Bandpass Filter
 * 保留特定頻率範圍
 */

class AudioBandpassFilter {
    constructor() {
    ...
    initElements() {
        ...
        this.centerFreqSlider = document.getElementById('centerFreq');
        this.bandwidthSlider = document.getElementById('bandwidth');
        this.qSlider = document.getElementById('qValue');
        this.mixSlider = document.getElementById('mix');

        this.centerFreqValue = document.getElementById('centerFreqValue');
    ...
    bindEvents() {
        ...
        // Sliders
        this.centerFreqSlider.addEventListener('input', () => {
            const freq = parseInt(this.centerFreqSlider.value);
            this.centerFreqValue.textContent = freq >= 1000
                ? `${(freq / 1000).toFixed(1)} kHz`
                : `${freq} Hz`;
            this.drawFilterResponse();
        });
        this.bandwidthSlider.addEventListener('input', () => {
            const bw = parseInt(this.bandwidthSlider.value) / 10;
            this.bandwidthValue.textContent = `${bw.toFixed(1)} oct`;
            this.drawFilterResponse();
        });
    ...
    drawFilterResponse() {
        ...
        const centerFreq = parseInt(this.centerFreqSlider.value);
        const Q = parseInt(this.qSlider.value) / 10;
        ...
        for (let x = 0; x < width; x++) {
            ...
            const magnitude = this.calculateBandpassMagnitude(freq, centerFreq, Q, sampleRate);
        ...
        const centerX = this.freqToX(centerFreq, width);
        ...
        const centerLabel = centerFreq >= 1000 ? `${(centerFreq/1000).toFixed(1)}kHz` : `${centerFreq}Hz`;
        this.ctx.fillText(centerLabel, centerX + 5, 15);

        // Draw bandwidth markers
        const bandwidth = parseInt(this.bandwidthSlider.value) / 10;
    ...
    calculateBandpassMagnitude(freq, centerFreq, Q, sampleRate) {
        // Bandpass filter magnitude response
        const ratio = freq / centerFreq;
        const bandwidth = 1 / Q;

        // Second-order bandpass response
        const numerator = bandwidth * ratio;
        const denominator = Math.sqrt(
            Math.pow(1 - ratio * ratio, 2) + Math.pow(bandwidth * ratio, 2)
        );
    ...
    async processAudio() {
        ...
        try {
            const processedBuffer = await this.applyBandpass(
                this.audioBuffer,
                centerFreq,
                Q,
                mix
    ...
    async applyBandpass(buffer, centerFreq, Q, mix) {
        ...
        // Calculate bandpass biquad coefficients
        const coeffs = this.calculateBiquadCoeffs(centerFreq, Q, sampleRate);
        ...
        for (let channel = 0; channel < channels; channel++) {
            ...
            for (let i = 0; i < length; i++) {
                ...
                // Mix dry and wet
                let sample = drySample * (1 - mix) + y0 * mix;
    ...
    calculateBiquadCoeffs(centerFreq, Q, sampleRate) {
        // Bandpass biquad filter coefficients (constant 0dB peak gain)
    ...
    showResult() {
        ...
        const centerFreq = parseInt(this.centerFreqSlider.value);
        const centerStr = centerFreq >= 1000 ? `${(centerFreq/1000).toFixed(1)} kHz` : `${centerFreq} Hz`;
        const Q = (parseInt(this.qSlider.value) / 10).toFixed(1);
        this.resultInfo.textContent = `中心頻率: ${centerStr} | Q 值: ${Q} | 格式: ${this.outputFormat.toUpperCase()} | 大小: ${size} KB`;
    ...
    downloadResult() {
        ...
        a.href = url;
        a.download = `${baseName}_bandpass.${ext}`;
        a.click();
