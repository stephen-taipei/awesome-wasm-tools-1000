/**
 * AUD-020 Audio Dynamic Range Compressor
...
const makeupValue = document.getElementById('makeupValue');
...
const resultMakeup = document.getElementById('resultMakeup');
...
// Presets
const presets = {
  light: { threshold: -20, ratio: 2, makeup: 3, attack: 20, release: 200 },
  normal: { threshold: -24, ratio: 4, makeup: 6, attack: 10, release: 150 },
  heavy: { threshold: -30, ratio: 8, makeup: 12, attack: 5, release: 100 },
  vocal: { threshold: -18, ratio: 3, makeup: 4, attack: 15, release: 200 },
  broadcast: { threshold: -16, ratio: 6, makeup: 8, attack: 3, release: 100 }
};
...
function applyCompression(buffer, thresholdDb, ratioVal, makeupDb, attackMs, releaseMs) {
  const ctx = getAudioContext();
  ...
  // These determine how quickly the compressor responds
  ...
  for (let ch = 0; ch < numChannels; ch++) {
    ...
    let envelope = 0; // Current envelope follower value
    ...
    for (let i = 0; i < length; i++) {
      ...
      if (inputLevel > envelope) {
        ...
        // Release phase - envelope falls slowly
      ...
      if (envelope > thresholdLinear) {
        // Calculate how much above threshold (in dB)
        const overThresholdDb = linearToDb(envelope) - thresholdDb;

        // Apply ratio - reduce the amount over threshold
        const reducedOverThreshold = overThresholdDb / ratioVal;

        // Calculate target level
        const targetDb = thresholdDb + reducedOverThreshold;
      ...
      outputData[i] = inputSample * gain * makeupLinear;
...
function updateValueDisplays() {
  ...
  ratioValue.textContent = ratio.value + ':1';
  makeupValue.textContent = makeup.value + ' dB';
...
async function processAudio() {
  ...
  const makeupDb = parseFloat(makeup.value);
  const attackMs = parseFloat(attack.value);
  ...
  try {
    updateProgress(25, '套用動態壓縮...');

    // Use setTimeout to allow UI to update
    await new Promise(resolve => setTimeout(resolve, 10));

    processedBuffer = applyCompression(audioBuffer, thresholdDb, ratioVal, makeupDb, attackMs, releaseMs);
    ...
    resultThreshold.textContent = thresholdDb + ' dB';
    resultRatio.textContent = ratioVal + ':1';
    resultMakeup.textContent = makeupDb + ' dB';
...
threshold.addEventListener('input', updateValueDisplays);
ratio.addEventListener('input', updateValueDisplays);
makeup.addEventListener('input', updateValueDisplays);
attack.addEventListener('input', updateValueDisplays);
