/**
 * AUD-065 音頻高通濾波 - Audio Highpass Filter
 * 削減低頻保留高頻
 */
...
class AudioHighpassFilter {
    ...
    calculateMagnitude(freq, cutoff, order, Q, sampleRate) {
        // Butterworth highpass response with optional resonance
        const ratio = cutoff / freq; // Inverted ratio for highpass
        ...
        if (order === 1) {
            ...
            const freqRatio = freq / cutoff;
            const resonancePeak = freqRatio > 0.5 && freqRatio < 2 ?
                1 + Q * Math.exp(-Math.pow(Math.log(freqRatio), 2) * 2) : 1;
    ...
    async processAudio() {
        ...
        try {
            const processedBuffer = await this.applyHighpass(
                this.audioBuffer,
                cutoff,
                resonance,
                this.slope,
    ...
    async applyHighpass(buffer, cutoff, resonance, slope, mix) {
    ...
    calculateBiquadCoeffs(cutoff, Q, sampleRate) {
        // Highpass biquad filter coefficients
    ...
    downloadResult() {
        ...
        a.href = url;
        a.download = `${baseName}_highpass.${ext}`;
        a.click();
