/**
 * AUD-083: Sawtooth Wave Generator
 */
...
generateBtn.addEventListener('click', generateSawtoothWave);
...
function generateSawtoothWave() {
  ...
  setTimeout(() => {
    ...
    generateBtn.innerHTML = '<span>🎵</span> 生成鋸齒波';
  }, 100);
}
...
function drawWaveform(vol) {
  ...
  for (let x = 0; x < width; x++) {
    ...
    const y = height / 2 - (2 * phase - 1) * (height / 2 - 10) * vol;
...
function downloadResult() {
  ...
  link.download = `sawtooth_${freqInput.value}Hz.wav`;
  link.click();
}
