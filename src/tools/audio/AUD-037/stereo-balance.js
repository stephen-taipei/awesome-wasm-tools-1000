...
function updateBalanceDisplay(value) {
  if (value === 0) {
    balanceValue.textContent = '中心 (0)';
  } else if (value < 0) {
    balanceValue.textContent = `偏左 (${value})`;
  } else {
    balanceValue.textContent = `偏右 (+${value})`;
  }
}

// 更新音量計顯示
function updateMeterDisplay(balance) {
  // 計算左右聲道增益
  const { leftGain, rightGain } = calculateGains(balance);

  leftMeter.style.height = (leftGain * 100) + '%';
  rightMeter.style.height = (rightGain * 100) + '%';

  leftValue.textContent = Math.round(leftGain * 100) + '%';
  rightValue.textContent = Math.round(rightGain * 100) + '%';
}
...
// 計算左右聲道增益
function calculateGains(balance) {
  // balance: -100 到 100
  // -100: 全左 (L=1, R=0)
  // 0: 置中 (L=1, R=1)
  // 100: 全右 (L=0, R=1)
  ...
  if (balance <= 0) {
    leftGain = 1;
    rightGain = 1 + balance / 100; // 0 到 1
  } else {
    leftGain = 1 - balance / 100; // 1 到 0
    rightGain = 1;
  }
 ...
 * 套用立體聲平衡
 */
function applyBalance(buffer, balance) {
...
async function processAudio() {
  ...
  try {
    ...
    progressText.textContent = '套用平衡調整...';
    await new Promise(resolve => setTimeout(resolve, 50));

    // 套用平衡
    const outputBuffer = applyBalance(originalBuffer, balance);
...
function showResults(results) {
  if (results.balance === 0) {
    resultBalance.textContent = '中心';
  } else if (results.balance < 0) {
    resultBalance.textContent = `L ${Math.abs(results.balance)}`;
  } else {
    resultBalance.textContent = `R ${results.balance}`;
  }
...
function downloadAudio() {
  ...
  const balanceStr = balance === '0' ? 'center' :
                     balance < 0 ? `L${Math.abs(balance)}` : `R${balance}`;
  const downloadName = `${originalName}_balance_${balanceStr}.${ext}`;
