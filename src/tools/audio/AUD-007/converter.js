/**
 * AUD-007 WMA Converter
 * Converts between WMA, WAV, and MP3 formats
 * Uses Web Audio API for decoding and lamejs for MP3 encoding
 * Note: WMA decoding depends on browser support (limited in most browsers)
 */
...
async function decodeAudioFile(file) {
  ...
  try {
    ...
    // WMA may not be supported in all browsers
    if (getFileExtension(file.name) === 'wma') {
      throw new Error('您的瀏覽器不支援 WMA 解碼。請使用 Chrome 或 Firefox，或先用其他工具將 WMA 轉換為 WAV/MP3。');
    }
    throw error;
  }
}
...
async function handleFile(file) {
  ...
  if (!['wma', 'wav', 'wave', 'mp3'].includes(ext)) {
    alert('請選擇 WMA、WAV 或 MP3 格式的音訊檔案');
    return;
  }
