/**
 * CMP-011: TAR.GZ Decompression
 *
 * Extracts files from TAR.GZ archives.
 * Uses pako for gzip decompression, then parses TAR.
...
class TarGzDecompressor {
  ...
  processFile(file) {
    ...
    if (!name.endsWith('.tar.gz') && !name.endsWith('.tgz')) {
      this.showStatus('error', '請選擇 TAR.GZ 或 TGZ 格式的檔案');
      return;
    }
  ...
  async extract() {
    if (!this.archiveFile) {
      this.showStatus('error', '請先選擇 TAR.GZ 檔案');
      return;
    }
    ...
    try {
      ...
      this.updateProgress(30, 'GZIP 解壓縮中...');
  ...
  async downloadAll() {
    ...
    link.download = this.archiveFile.name.replace(/\.(tar\.gz|tgz)$/i, '_extracted.zip');
    link.click();
    URL.revokeObjectURL(link.href);
  }
...
document.addEventListener('DOMContentLoaded', () => {
  window.decompressor = new TarGzDecompressor();
});

export default TarGzDecompressor;
