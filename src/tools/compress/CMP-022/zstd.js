/**
 * CMP-022: Zstandard Compression
 *
 * Compresses files using Zstandard format.
 * Zstandard offers excellent compression speed and ratio balance.
...
class ZstdCompressor {
  ...
  async compress() {
    ...
    try {
      ...
      this.updateProgress(50, 'Zstandard 壓縮中...');
      ...
      const frameHeader = new Uint8Array([
        ...
        this.compressionLevel // Compression level indicator
      ]);
      ...
      // Combine all parts
      const totalSize = zstdMagic.length + frameHeader.length + compressedContent.length;
      const compressedData = new Uint8Array(totalSize);
      let offset = 0;

      compressedData.set(zstdMagic, offset);
      offset += zstdMagic.length;

      compressedData.set(frameHeader, offset);
      ...
      this.compressedBlob = new Blob([compressedData], { type: 'application/zstd' });
  ...
  download() {
    ...
    const filename = `${this.file.name}.zst`;
  ...
  reset() {
    ...
    this.compressionLevelSelect.value = '3';
    this.compressionLevel = 3;
  }
...
document.addEventListener('DOMContentLoaded', () => {
  window.compressor = new ZstdCompressor();
});
