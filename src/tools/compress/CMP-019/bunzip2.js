/**
 * CMP-019: BZIP2 Decompression
 *
 * Decompresses BZIP2 files.
 * All processing is done locally in the browser.
 */

class Bunzip2Decompressor {
  constructor() {
    this.file = null;
  ...
  processFile(file) {
    if (!file.name.toLowerCase().endsWith('.bz2')) {
      this.showStatus('error', '請選擇 .bz2 格式的檔案');
      return;
    }
  ...
  async decompress() {
    if (!this.file) {
      this.showStatus('error', '請先選擇 .bz2 檔案');
      return;
    }
    ...
    try {
      ...
      if (uint8Array[0] !== 0x42 || uint8Array[1] !== 0x5A || uint8Array[2] !== 0x68) {
        throw new Error('Invalid BZIP2 file format');
      }

      this.updateProgress(50, 'BZIP2 解壓縮中...');

      // Skip BZ2 header (4 bytes: "BZh" + level)
      ...
      this.outputName = this.file.name.replace(/\.bz2$/i, '');
...
document.addEventListener('DOMContentLoaded', () => {
  window.decompressor = new Bunzip2Decompressor();
});

export default Bunzip2Decompressor;
