/**
 * CMP-023: Zstandard Decompression
 *
 * Decompresses Zstandard files.
 * All processing is done locally in the browser.
 */

class UnzstdDecompressor {
  constructor() {
    this.file = null;
  ...
  processFile(file) {
    ...
    if (!name.endsWith('.zst') && !name.endsWith('.zstd')) {
      this.showStatus('error', '請選擇 .zst 或 .zstd 格式的檔案');
      return;
    }
  ...
  async decompress() {
    ...
    try {
      ...
      if (uint8Array[0] !== 0x28 || uint8Array[1] !== 0xB5 ||
          uint8Array[2] !== 0x2F || uint8Array[3] !== 0xFD) {
        throw new Error('Invalid Zstandard file format');
      }

      this.updateProgress(50, 'Zstandard 解壓縮中...');

      // Skip magic (4 bytes) + frame header (2 bytes)
      const compressedContent = uint8Array.slice(6);
      ...
      this.outputName = this.file.name.replace(/\.(zst|zstd)$/i, '');
...
document.addEventListener('DOMContentLoaded', () => {
  window.decompressor = new UnzstdDecompressor();
});

export default UnzstdDecompressor;
