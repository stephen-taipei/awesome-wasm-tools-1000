/**
 * CMP-001: ZIP Compression
 *
 * Uses JSZip library to compress multiple files into a ZIP archive.
...
class ZipCompressor {
  ...
  async compress() {
    ...
    try {
      ...
      for (let i = 0; i < totalFiles; i++) {
        ...
        this.updateProgress((i + 1) / totalFiles * 80, `處理中: ${file.name}`);
      }

      this.updateProgress(85, '生成 ZIP 檔案...');
  ...
  download() {
    ...
    const filename = `${this.outputFilename.value || 'archive'}.zip`;
...
document.addEventListener('DOMContentLoaded', () => {
  window.compressor = new ZipCompressor();
});
