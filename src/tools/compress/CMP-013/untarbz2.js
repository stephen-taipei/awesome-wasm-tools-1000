/**
 * CMP-013: TAR.BZ2 Decompression
 *
 * Extracts files from TAR.BZ2 archives.
 * All processing is done locally in the browser.
 */

class TarBz2Decompressor {
  constructor() {
  ...
  processFile(file) {
    ...
    if (!name.endsWith('.tar.bz2') && !name.endsWith('.tbz2') && !name.endsWith('.bz2')) {
      this.showStatus('error', '請選擇 TAR.BZ2 格式的檔案');
      return;
    }
  ...
  async extract() {
    if (!this.archiveFile) {
      this.showStatus('error', '請先選擇 TAR.BZ2 檔案');
      return;
    }
    ...
    try {
      ...
      // Check BZ2 magic header
      if (uint8Array[0] !== 0x42 || uint8Array[1] !== 0x5A) {
        throw new Error('Invalid BZ2 file format');
      }

      this.updateProgress(30, 'BZIP2 解壓縮中...');

      // Skip BZ2 header and decompress
      let tarData;
      const compressedData = uint8Array.slice(10); // Skip BZ2 header
      ...
      if (typeof DecompressionStream !== 'undefined') {
        try {
          ...
          // Fallback: use raw data without header
  ...
  async downloadAll() {
    ...
    link.download = this.archiveFile.name.replace(/\.(tar\.bz2|tbz2)$/i, '_extracted.zip');
    link.click();
    URL.revokeObjectURL(link.href);
  }
...
document.addEventListener('DOMContentLoaded', () => {
  window.decompressor = new TarBz2Decompressor();
});

export default TarBz2Decompressor;
