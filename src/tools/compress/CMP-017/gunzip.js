/**
 * CMP-017: GZIP Decompression
 *
 * Decompresses GZIP files.
 * Uses pako library for gunzip.
 * All processing is done locally in the browser.
 */

class GunzipDecompressor {
  constructor() {
    this.file = null;
  ...
  processFile(file) {
    if (!file.name.toLowerCase().endsWith('.gz')) {
      this.showStatus('error', '請選擇 .gz 格式的檔案');
      return;
    }
  ...
  async decompress() {
    if (!this.file) {
      this.showStatus('error', '請先選擇 .gz 檔案');
      return;
    }
    ...
    try {
      ...
      this.updateProgress(50, 'GZIP 解壓縮中...');
      ...
      this.outputName = this.file.name.replace(/\.gz$/i, '');
      if (this.outputName === this.file.name) {
        this.outputName = this.file.name + '.decompressed';
      }

      // Try to determine MIME type from extension
      const ext = this.outputName.split('.').pop().toLowerCase();
      const mimeTypes = {
        'txt': 'text/plain',
        'html': 'text/html',
        'css': 'text/css',
      ...
      this.decompressedBlob = new Blob([decompressedData], { type: mimeType });
...
document.addEventListener('DOMContentLoaded', () => {
  window.decompressor = new GunzipDecompressor();
});

export default GunzipDecompressor;
