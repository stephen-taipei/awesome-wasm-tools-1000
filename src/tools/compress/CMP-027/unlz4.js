/**
 * CMP-027: LZ4 Decompression
 *
 * Decompresses LZ4 files.
 * All processing is done locally in the browser.
 */

class UnLZ4Decompressor {
  constructor() {
    this.file = null;
  ...
  processFile(file) {
    if (!file.name.toLowerCase().endsWith('.lz4')) {
      this.showStatus('error', '請選擇 .lz4 格式的檔案');
      return;
    }
  ...
  async decompress() {
    ...
    try {
      ...
      // Check LZ4 frame magic: 0x184D2204 (little endian: 04 22 4D 18)
      if (uint8Array[0] !== 0x04 || uint8Array[1] !== 0x22 ||
          uint8Array[2] !== 0x4D || uint8Array[3] !== 0x18) {
        throw new Error('Invalid LZ4 file format');
      }

      this.updateProgress(50, 'LZ4 解壓縮中...');

      // Skip magic (4) + frame descriptor (3) and remove end mark (4)
      const compressedContent = uint8Array.slice(7, -4);
      ...
      this.outputName = this.file.name.replace(/\.lz4$/i, '');
...
document.addEventListener('DOMContentLoaded', () => {
  window.decompressor = new UnLZ4Decompressor();
});

export default UnLZ4Decompressor;
