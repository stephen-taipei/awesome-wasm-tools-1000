/**
 * CMP-021: XZ/LZMA Decompression
 *
 * Decompresses XZ files.
 * All processing is done locally in the browser.
 */

class UnxzDecompressor {
  constructor() {
    this.file = null;
  ...
  processFile(file) {
    ...
    if (!name.endsWith('.xz') && !name.endsWith('.lzma')) {
      this.showStatus('error', '請選擇 .xz 或 .lzma 格式的檔案');
      return;
    }
  ...
  async decompress() {
    if (!this.file) {
      this.showStatus('error', '請先選擇 .xz 檔案');
      return;
    }
    ...
    try {
      ...
      // Verify XZ header: FD 37 7A 58 5A 00
      if (uint8Array[0] !== 0xFD || uint8Array[1] !== 0x37 ||
          uint8Array[2] !== 0x7A || uint8Array[3] !== 0x58) {
        throw new Error('Invalid XZ file format');
      }

      this.updateProgress(50, 'XZ/LZMA 解壓縮中...');

      // Skip XZ header (6 bytes) + stream flags (2 bytes)
      const compressedContent = uint8Array.slice(8, -6); // Remove footer too
      ...
      this.outputName = this.file.name.replace(/\.(xz|lzma)$/i, '');
      ...
      this.outputFilename.textContent = this.outputName;
...
document.addEventListener('DOMContentLoaded', () => {
  window.decompressor = new UnxzDecompressor();
});

export default UnxzDecompressor;
