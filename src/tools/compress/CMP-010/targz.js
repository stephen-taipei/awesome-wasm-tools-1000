/**
 * CMP-010: TAR.GZ Compression
 *
 * Creates TAR.GZ archives (gzipped tar files) from multiple files.
...
class TarGzCompressor {
  ...
  async compress() {
    ...
    try {
      ...
      this.updateProgress(60, 'GZIP 壓縮中...');

      // Compress with gzip using pako
      let gzippedData;
      if (typeof pako !== 'undefined') {
        gzippedData = pako.gzip(tarArray, { level: this.compressionLevel });
      } else if (typeof CompressionStream !== 'undefined') {
        // Fallback to CompressionStream
        ...
        gzippedData = new Uint8Array(totalLen);
        let off = 0;
        for (const chunk of chunks) {
          gzippedData.set(chunk, off);
          off += chunk.length;
        }
      } else {
      ...
      this.compressedBlob = new Blob([gzippedData], { type: 'application/gzip' });
  ...
  download() {
    ...
    const filename = `${this.outputFilename.value || 'archive'}.tar.gz`;
...
document.addEventListener('DOMContentLoaded', () => {
  window.compressor = new TarGzCompressor();
});

export default TarGzCompressor;
