class SpherizeTool {
    ...
    for (let y = 0; y < img.height; y++) {
      for (let x = 0; x < img.width; x++) {
        ...
        if (distance < 1 && strength !== 0) {
          // Spherize distortion formula
          let newDistance;
          if (strength > 0) {
            // Bulge outward
            newDistance = Math.asin(distance * strength) / (Math.PI / 2 * strength);
          } else {
            // Pinch inward
            newDistance = Math.sin(distance * Math.PI / 2 * (-strength)) / (-strength);
          }

          if (!isNaN(newDistance) && isFinite(newDistance)) {
            const scale = distance > 0 ? newDistance / distance : 1;
  ...
  reset() {
    ...
    document.querySelectorAll('.mode-tab').forEach(t => t.classList.remove('active'));
    document.querySelector('.mode-tab[data-mode="normal"]').classList.add('active');
