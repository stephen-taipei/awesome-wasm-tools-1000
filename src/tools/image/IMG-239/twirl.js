...
class TwirlTool {
  constructor() {
    ...
    this.settings = { angle: 90, radius: 80, centerX: 50, centerY: 50 };
    this.init();
  }
  ...
  bindEvents() {
    ...
    ['radius', 'centerX', 'centerY'].forEach(id => {
  ...
  render() {
    ...
    const { angle, radius, centerX, centerY } = this.settings;
    ...
    for (let y = 0; y < h; y++) {
      for (let x = 0; x < w; x++) {
        ...
        if (dist <= maxRadius && maxRadius > 0) {
          // Calculate twirl angle based on distance from center
          const twirlFactor = 1 - (dist / maxRadius);
          const twirlAngle = twirlFactor * twirlFactor * maxAngle;
  ...
  download() {
    ...
    link.download = `twirl_${Date.now()}.png`;
    link.href = this.canvas.toDataURL('image/png');
    link.click();
  }
}

document.addEventListener('DOMContentLoaded', () => new TwirlTool());
