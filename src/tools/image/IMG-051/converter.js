...
class HorizontalMergeTool {
  ...
  updatePreview() {
    ...
    this.images.forEach(item => {
      ...
      maxHeight = Math.max(maxHeight, item.image.height);
    });

    totalWidth += gap * (this.images.length - 1);
    ...
    this.previewCtx.fillRect(0, 0, totalWidth, maxHeight);
    ...
    this.images.forEach(item => {
      ...
      if (this.align === 'center') {
        y = (maxHeight - item.image.height) / 2;
      } else if (this.align === 'bottom') {
        y = maxHeight - item.image.height;
      }

      this.previewCtx.drawImage(item.image, x, y);
      x += item.image.width + gap;
    });

    this.previewInfo.textContent = `${totalWidth} × ${maxHeight}`;
  }
  ...
  download() {
    ...
    link.download = `horizontal_merge_${Date.now()}.${ext}`;
    link.click();
