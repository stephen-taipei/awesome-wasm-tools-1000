class SpherizeTool {
  ...
  render() {
    ...
    const isBulge = this.settings.mode === 'bulge';
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        if (dist < maxRadius && dist > 0) {
          ...
          let newDist;
          if (isBulge) {
            // Bulge effect - spherical projection
            const factor = Math.sin(normalizedDist * Math.PI / 2);
            newDist = dist * (1 - strength * (1 - factor));
          } else {
            // Pinch effect - inverse spherical
            const factor = Math.pow(normalizedDist, 2);
            newDist = dist * (1 + strength * (factor - 1));
          }
  ...
  download() {
    ...
    link.download = 'spherize-image.png';
    link.href = this.canvas.toDataURL('image/png');
    link.click();
  }

  reset() {
    this.settings = { mode: 'bulge', strength: 50, radius: 100, centerX: 50, centerY: 50 };
    document.querySelectorAll('.mode-btn').forEach(b => b.classList.remove('active'));
    document.querySelector('.mode-btn[data-mode="bulge"]').classList.add('active');
