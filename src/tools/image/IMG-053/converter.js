...
class NineGridTool {
  ...
  createGrid() {
    ...
    for (let i = 0; i < 9; i++) {
      ...
      placeholder.innerHTML = `<span class="cell-number">${i + 1}</span><br>點擊上傳`;
  ...
  updateCell(index) {
    ...
    if (data) {
      ...
      cell.innerHTML = `
        <input type="file" accept="image/png,image/jpeg,image/webp" data-index="${index}">
      ...
      cell.innerHTML = `
        <input type="file" accept="image/png,image/jpeg,image/webp" data-index="${index}">
        <div class="cell-placeholder"><span class="cell-number">${index + 1}</span><br>點擊上傳</div>
      `;
  ...
  updateUI() {
    ...
    if (imageCount > 0 && imageCount < 9) {
      this.showStatus('info', `已上傳 ${imageCount}/9 張圖片，空白格子將使用背景色填充`);
    } else {
  ...
  updatePreview() {
    ...
    const totalSize = parseInt(this.sizeSlider.value);
    const bgColor = this.bgColor.value;

    const cellSize = (totalSize - gap * 4) / 3;

    this.previewCanvas.width = totalSize;
    this.previewCanvas.height = totalSize;

    // Fill background
    this.previewCtx.fillStyle = bgColor;
    this.previewCtx.fillRect(0, 0, totalSize, totalSize);
    ...
    for (let i = 0; i < 9; i++) {
      ...
      if (this.cells[i]) {
        this.drawRoundedImage(this.cells[i].image, x, y, cellSize, cellSize, radius);
      }
    }

    this.previewInfo.textContent = `${totalSize} × ${totalSize}`;
  }
  ...
  async generate() {
    ...
    try {
      // Preview already has the merged result
      ...
      this.showStatus('success', `九宮格已生成！（${imageCount}/9 張圖片）`);
  ...
  download() {
    ...
    link.download = `nine_grid_${Date.now()}.${ext}`;
    link.click();
  ...
  reset() {
    ...
    this.sizeSlider.value = 1200;
    this.sizeValue.textContent = '1200px';
