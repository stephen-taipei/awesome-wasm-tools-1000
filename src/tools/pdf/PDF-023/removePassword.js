/**
 * PDF-023: PDF Password Remove
 */
class PDFPasswordRemove {
  ...
  bindEvents() {
    ...
    this.processBtn.addEventListener('click', () => this.removePassword());
  ...
  async removePassword() {
    ...
    try {
      ...
      const pdfDoc = await PDFLib.PDFDocument.load(arrayBuffer, { password: this.currentPassword.value, ignoreEncryption: true });
      this.updateProgress(70, '移除密碼...');
      ...
      setTimeout(() => {
        ...
        this.showStatus('success', '密碼移除完成！');
  ...
  download() {
    ...
    link.download = this.file.name.replace(/\.pdf$/i, '_unlocked.pdf');
    link.click();
    URL.revokeObjectURL(link.href);
  }
  reset() {
    this.file = null; this.outputBytes = null; this.fileInput.value = ''; this.currentPassword.value = '';
...
document.addEventListener('DOMContentLoaded', () => { window.pdfPasswordRemove = new PDFPasswordRemove(); });
export default PDFPasswordRemove;
