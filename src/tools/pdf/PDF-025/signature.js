...
class PDFSignature {
  ...
  bindEvents() {
    ...
    this.processBtn.addEventListener('click', () => this.addSignature());
    this.downloadBtn.addEventListener('click', () => this.download());
    this.resetBtn.addEventListener('click', () => this.reset());
  }
  processFile(file) {
    if (file.type !== 'application/pdf') { this.showStatus('error', '請選擇 PDF 文件'); return; }
    this.file = file; this.fileInfo.textContent = file.name;
    this.settingsPanel.style.display = 'block'; this.processBtn.disabled = false;
    this.showStatus('success', `已載入: ${file.name}`);
  }
  async addSignature() {
    if (!this.file) return;
    const startTime = performance.now();
    this.progressContainer.classList.add('active'); this.processBtn.disabled = true;
    try {
      this.updateProgress(30, '讀取 PDF...');
      const arrayBuffer = await this.file.arrayBuffer();
      const pdfDoc = await PDFLib.PDFDocument.load(arrayBuffer);
      this.updateProgress(60, '添加簽章資訊...');
      pdfDoc.setAuthor(this.signerName.value || 'Signer');
      pdfDoc.setSubject(this.reason.value || 'Digital Signature');
      ...
      setTimeout(() => {
        ...
        this.showStatus('info', '已添加簽章資訊（完整數位簽章需要憑證）');
        this.downloadBtn.style.display = 'inline-flex'; this.resetBtn.style.display = 'inline-flex';
  ...
  download() {
    ...
    link.download = this.file.name.replace(/\.pdf$/i, '_signed.pdf');
    link.click(); URL.revokeObjectURL(link.href);
  }
  reset() {
    this.file = null; this.outputBytes = null; this.fileInput.value = '';
    this.signerName.value = ''; this.reason.value = '';
...
document.addEventListener('DOMContentLoaded', () => { window.pdfSignature = new PDFSignature(); });
