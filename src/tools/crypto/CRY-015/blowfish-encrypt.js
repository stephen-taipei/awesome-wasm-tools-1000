/**
 * CRY-015: Blowfish Encryption Tool
 *
 * Implements Blowfish encryption algorithm in JavaScript.
 * Blowfish is a symmetric-key block cipher with variable key length (32-448 bits).
 * All processing is done locally in the browser.
 *
 * Note: Blowfish is considered legacy. Use AES or ChaCha20 for new applications.
 */
...
class BlowfishEncryptor {
  ...
  init() {
    ...
    // Initialize Blowfish S-boxes and P-array
    this.initBlowfish();
    this.bindEvents();
  }
  ...
  initBlowfish() {
    ...
    // Blowfish S-boxes (4 boxes, 256 entries each)
    this.S = [
  ...
  // Key expansion
  expandKey(key) {
    // Reset P and S to initial values
    this.initBlowfish();
  ...
  async encrypt() {
    ...
    try {
      ...
      // Pad plaintext to 8-byte blocks (PKCS7)
      const blockSize = 8;
      ...
      for (let i = 0; i < paddedData.length; i += 8) {
        let L = (paddedData[i] << 24) | (paddedData[i+1] << 16) | (paddedData[i+2] << 8) | paddedData[i+3];
        let R = (paddedData[i+4] << 24) | (paddedData[i+5] << 16) | (paddedData[i+6] << 8) | paddedData[i+7];
        ...
        if (mode === 'CBC') {
          prevBlock = new Uint32Array([L, R]);
        }

        encrypted[i] = (L >>> 24) & 0xff;
        encrypted[i+1] = (L >>> 16) & 0xff;
        encrypted[i+2] = (L >>> 8) & 0xff;
        encrypted[i+3] = L & 0xff;
        encrypted[i+4] = (R >>> 24) & 0xff;
        encrypted[i+5] = (R >>> 16) & 0xff;
        encrypted[i+6] = (R >>> 8) & 0xff;
        encrypted[i+7] = R & 0xff;
      }
      ...
      this.showStatus('success', 'Blowfish 加密完成！');
    } catch (error) {
...
document.addEventListener('DOMContentLoaded', () => {
  window.blowfishEncryptor = new BlowfishEncryptor();
});

export default BlowfishEncryptor;
