/**
 * CRY-027: BLAKE2 Hash Tool
 * Uses noble-hashes library for BLAKE2b/BLAKE2s hash computation.
 */

class BLAKE2Hash {
  constructor() { this.blake2 = null; this.init(); }

  async init() {
    try { this.blake2 = await import('https://cdn.jsdelivr.net/npm/@noble/hashes@1.3.2/+esm'); } catch (e) { console.warn('BLAKE2 library load failed'); }
  ...
  async hash() {
    if (!this.blake2) { this.showStatus('error', 'BLAKE2 函式庫載入失敗'); return; }
    let data, dataLength;
    ...
    try {
      ...
      const hash = variant === 'blake2b' ? this.blake2.blake2b(data) : this.blake2.blake2s(data);
      ...
      this.showStatus('success', `${variant.toUpperCase()} 雜湊計算完成！`);
...
document.addEventListener('DOMContentLoaded', () => { window.blake2Hash = new BLAKE2Hash(); });
