/**
 * CRY-010: ECDSA Signature Verification Tool
 *
 * Uses Web Crypto API to verify ECDSA digital signatures.
  ...
  async verify() {
    ...
    try {
      ...
      if (isValid) {
        ...
        this.resultText.textContent = 'ECDSA 簽章驗證成功！訊息是真實且完整的。';
        this.showStatus('success', '簽章驗證通過！');
      } else {
        this.verifyResult.className = 'result-box error';
        this.resultIcon.textContent = '✗';
        this.resultText.textContent = 'ECDSA 簽章驗證失敗！訊息可能被竄改或簽章無效。';
...
document.addEventListener('DOMContentLoaded', () => {
  window.ecdsaVerifier = new ECDSAVerifier();
});

export default ECDSAVerifier;
