/**
 * CRY-024: SHA-384 Hash Tool
 * Uses Web Crypto API. SHA-384 produces a 384-bit (48-byte) hash value.
 */
...
class SHA384Hash {
  ...
  async hash() {
    ...
    try {
      const hash = new Uint8Array(await crypto.subtle.digest('SHA-384', data));
      ...
      this.showStatus('success', 'SHA-384 雜湊計算完成！');
...
document.addEventListener('DOMContentLoaded', () => { window.sha384Hash = new SHA384Hash(); });
