/**
 * CRY-028: BLAKE3 Hash Tool
 * Uses noble-hashes library for BLAKE3 hash computation.
 * BLAKE3 is a cryptographic hash function that is faster than MD5 and SHA-1.
 */

class BLAKE3Hash {
  constructor() { this.blake3 = null; this.init(); }

  async init() {
    try { this.blake3 = await import('https://cdn.jsdelivr.net/npm/@noble/hashes@1.3.2/+esm'); } catch (e) { console.warn('BLAKE3 library load failed'); }
    ...
    this.inputLengthEl = document.getElementById('inputLength');
  ...
  async hash() {
    if (!this.blake3) { this.showStatus('error', 'BLAKE3 函式庫載入失敗'); return; }
    let data, dataLength;
    ...
    try {
      const outputLen = parseInt(this.outputLength.value);
      const hash = this.blake3.blake3(data, { dkLen: outputLen });
...
document.addEventListener('DOMContentLoaded', () => { window.blake3Hash = new BLAKE3Hash(); });
